...
public class JigaRTPCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0) {
            ...
            plugin.getRtpManager().forceRTP(target, world, success -> {
                if (success) {
                    ...
                    sender.sendMessage(
                            ChatColor.RED +
                            "Could not find a safe location for " +
                            target.getName() +
                            ". Please try again."
                    );
                }
        ...
        if (!(sender instanceof Player)) {
            sender.sendMessage(
                    ChatColor.RED +
                    "Only players can use this command. Console must specify a player: /jigartp <player>"
            );
            return true;
        }
        ...
        if (remaining > 0) {
            player.sendMessage(
                    ...
                    "You must wait " +
                    remaining +
                    " more second(s) before using /jigartp again."
            );
            return true;
        }
